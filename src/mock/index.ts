...
import type { CharStationRecord, PageCommonResp } from '@/api/charStation'
Mock.setup({
...
//充电站监控接口
let chargingStation:CharStationRecord[] = [
  {
    name: "北京西单充电站",
    id: "VXZ10001",
    city: "北京",
...
// 定义 API 接口
Mock.mock("https://www.demo.com/charStation/page", 'post', (options: any) => {
  chargingStation = originalChargingStation
  const { id, name, status, pageNo, pageSize } = options.body ? JSON.parse(options.body) : {}
  // 根据条件过滤数据
  if (id) {
...
//更新数据
Mock.mock("https://www.demo.com/charStation/update", 'post', (options: any) => {
  const data= options.body ? JSON.parse(options.body) : null;
  if(data){
    //数组根据id判断是否存在，不存在新增，存在更新
    const index=originalChargingStation.findIndex(item=>item.id===data.id);
    if(index===-1){
      originalChargingStation.push(data); 
    } else {
      originalChargingStation[index]=data; 
    }
  }
  return {
    code: 200,
...
//新增充电桩
Mock.mock("https://www.demo.com/charStation/add", 'post', (options: any) => {
  const data= options.body ? JSON.parse(options.body) : null;
  if(data){
    //数组根据id判断是否存在，不存在新增，存在更新
    const index=originalChargingStation.findIndex(item=>item.id===data.id);
...
//删除充电桩
Mock.mock("https://www.demo.com/charStation/delete", 'post', (options: any) => {
  console.log("删除充电桩", options);
  const data= options.body ? JSON.parse(options.body) : null;
  if(data){
    const index=originalChargingStation.findIndex(item=>item.id===data.id);
    if(index !== -1){
      originalChargingStation.splice(index, 1);
    } 
  }
  return {
    code: 200,
...
Mock.mock("https://www.demo.com/revenueList", 'post', (options: any) => {
  ...
  const { name = "", pageNo = 1, pageSize = 10 } = options.body ? JSON.parse(options.body) : {}
  if (name) {
...
let chargingPile = [
  {
    ...
    list: [
      { id: "CD1001", voltage: "314v", current: "212.2A", power: "21KW", tem: "32°c", status: 1,
        ...
        fixRecord:[
          {time:"12:08:17",msg:"维修1"},
          {time:"13:12:09",msg:"维修1"},
          {time:"13:15:22",msg:"维修1"},
          {time:"16:22:33",msg:"维修1"},
          {time:"17:27:17",msg:"维修1"},
          {time:"18:08:33",msg:"维修1"},
        ]
